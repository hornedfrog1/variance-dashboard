...
if page == "Variance Dashboard":
    ...
    st.success("Want to see how to build this? Open the sidebar and click 'How you can make this or something similar!'")
    ...
    c3.info("Swap in your own business case, product, and cost assumptions to personalize it.")
    ...
    with st.expander("Even more ideas to improve the experience"):
        st.markdown(
            """
            - Add a glossary page for terms like favorable, unfavorable, rate variance, and efficiency variance.
    ...
    with g1:
        with st.expander("Standard Cost"):
            st.write("The expected cost of producing a product under normal conditions. In this app, standard cost is the benchmark used to compare actual results.")
        with st.expander("Actual Cost"):
            st.write("The real cost incurred based on actual materials used, prices paid, labor hours worked, and wage rates.")
        with st.expander("Variance"):
            st.write("The difference between a standard amount and an actual amount. Variances help explain where performance differed from plan.")
        with st.expander("Favorable (F)"):
            st.write("A variance that lowers cost compared with standard. In this app, a negative variance amount is treated as favorable.")
        with st.expander("Unfavorable (U)"):
            st.write("A variance that increases cost compared with standard. In this app, a positive variance amount is treated as unfavorable.")
        with st.expander("Direct Materials"):
            st.write("Raw materials that can be directly traced to the finished product, such as aluminum used to make the enclosure.")

    with g2:
        with st.expander("Direct Labor"):
            st.write("The labor cost of employees who directly work on producing the product.")
        with st.expander("DM Price Variance"):
            st.write("The cost impact of paying a different price per unit of material than expected. Formula: AQ x (AP - SP).")
        with st.expander("DM Quantity Variance"):
            st.write("The cost impact of using more or fewer materials than the standard allowed for actual output. Formula: SP x (AQ - SQ).")
        with st.expander("DL Rate Variance"):
            st.write("The cost impact of paying a different labor rate per hour than expected. Formula: AH x (AR - SR).")
        with st.expander("DL Efficiency Variance"):
            st.write("The cost impact of using more or fewer labor hours than the standard allowed for actual output. Formula: SR x (AH - SH).")
        with st.expander("Waterfall Chart"):
            st.write("A visual that starts with a base value, then shows how each positive or negative step changes the total. Here it shows how standard cost becomes actual cost.")
    ...
    with a1:
        with st.expander("Streamlit"):
            st.write("A Python framework used to turn data analysis code into a simple interactive web app.")
        with st.expander("Plotly"):
            st.write("A visualization library used here to create the interactive waterfall chart.")
        with st.expander("Prompt"):
            st.write("A clear instruction given to ChatGPT to generate code, explanations, or deployment steps.")

    with a2:
        with st.expander("Deploy"):
            st.write("To publish the app online so other people can access it through a web link.")
        with st.expander("requirements.txt"):
            st.write("A text file listing the Python packages needed to run the app, such as streamlit and plotly.")
        with st.expander("GitHub"):
            st.write("A platform used to store and share code, and often used as the source for deploying an app.")
